...
def call_api(url, endpoint_params=''):
    ...
    try:
        if response['json_data']['error']['code'] == 4:
            limit = 'APIリミットに達しました。1時間後に再度試してみて下さい。'
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            for user_id in user_ids:
                ...
                try:
                    # アカウント情報取得
                    account_response, api_limit = get_account_info(params)
                    ...
                    if business_discovery['media_count'] <= media_count and business_discovery['followers_count'] >= followers_count:
                        if created_at == 0:  # 全てのアカウントの場合
                            created = None
                            ff_rate = round(100 * business_discovery['follows_count']/business_discovery['followers_count'])
                            follower_increase_rate = round(business_discovery['followers_count']/business_discovery['media_count'])
                            user_list.append([
                                user_id,
                            ...
                            pagenate_account_response, api_limit = get_pagenate_account_info(params)
                            # 最初に投稿された日を取得
            ...
            # user_dataにデータフレームのuser_dataを入れる
